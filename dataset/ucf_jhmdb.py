...
class UCF_JHMDB_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        # load a data
        img_name, video_clip, key_frame_target, clip_target = self.pull_item(index)

        return img_name, video_clip, key_frame_target, clip_target
    ...
    def pull_item(self, index):  # 返回 该帧图片的完整名称，视频片段，真实标注（字典形式)
        ...
        time_dif_list = time_dif_list[::-1]  # 按照时间递增排序
        ...
        for i in range(self.len_clip):
            ...
            time_dif_list_.append(time_dif)
        ...
        clip_target = [torch.cat([torch.ones([clip_target[_].shape[0], 1])*time_dif_list[_],
                                  (clip_target[_][:, -1]-1).unsqueeze(-1), torch.ones([clip_target[_].shape[0], 1]),
                                  clip_target[_][:, :-1]], -1)
                       for _ in range(len(clip_target)) if (len(clip_target[_]) and (time_dif_list[_] != 0))]
...
class UCF_JHMDB_VIDEO_Dataset(Dataset):
    ...
    def pull_item(self, index):
        ...
        # 采样率翻倍系数
        d = 1  # 1  video数据集是只用来测试video mAP的 不存在训练的情况
