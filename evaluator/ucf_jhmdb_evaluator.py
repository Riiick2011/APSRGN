...
class UCF_JHMDB_Evaluator(object):  # 评估器类
    ...
    def evaluate_frame_map(self, model, epoch=1, show_pr_curve=False):  # 评估frame mAP
        ...
        if not os.path.exists(inference_dir):  # 完整路径
            ...
            for batch_idx, (batch_img_name, batch_video_clip, batch_key_frame_target, _) in enumerate(self.testloader):
                ...
                with torch.no_grad():  # 表明当前计算不需要反向传播，使用之后，强制后边的内容不进行计算图的构建
                    ...
                    if self.track_mode:  # 所有批次大小自动重置为1
                        ...
                        batch_scores, batch_labels, batch_bboxes = model(batch_video_clip, track)
                ...
                if not self.frame_det_dir:  # 如果不存在视频数据集的帧级检测结果，则重新进行推断
                    ...
                    for video_id, video_name in enumerate(self.videolist):
                        ...
                        if self.track_mode:
                            video_name_last_det = ''
                        dets_video = {}
                        for batch_idx, (batch_img_name, batch_video_clip, batch_key_frame_target, _) \
