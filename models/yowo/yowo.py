...
from models.yowo.encoder import build_encoder
...
class YOWO(nn.Module):
    def __init__(self, 
        ...
        if self.track_mode:
            self.track_net = TrackNet(self.m_cfg)
    ...
    # 前向推断一次
    def forward_propagation(self, video_clips, track=None):
        """
        Input:
        ...
        if self.track_mode:
            ...
            feats, Track_Conf_Maps = self.track_net(track, feats)
        else:
            Track_Conf_Maps = None
    ...
    # 前向推断一次并进行后处理
    @torch.no_grad()
    def inference(self, video_clips, track=None):
        """
        Input:
            video_clips: (Tensor) -> [B, 3, T, H, W].
        return:
        ...
        if self.head_type == 'Headv2':
            conf_pred, cls_pred, bbox_pred, anchor_point, stride_tensor, _ = self.forward_propagation(video_clips, track)
        elif self.head_type == 'Headv8':
            score_pred, bbox_pred, dist_pred, anchor_point, stride_tensor, _ = self.forward_propagation(video_clips, track)
        else:
    ...
    # 该方法用于训练
    def forward(self, video_clips, track=None):
        """
        Input:
            video_clips: (Tensor) -> [B, 3, T, H, W].
        ...
        if not self.trainable:  # 前向推断一次，并进行后处理
            return self.inference(video_clips, track)
        else:  # 前向推断一次，返回计算损失所需的结果
            # Output
            if self.head_type == 'Headv2':
                conf_pred, cls_pred, bbox_pred, anchor_point, stride_tensor, Track_Conf_Maps = self.forward_propagation(video_clips, track)
                # output dict
                ...
                score_pred, bbox_pred, dist_pred, anchor_point, stride_tensor, Track_Conf_Maps = self.forward_propagation(video_clips, track)
                # output dict
